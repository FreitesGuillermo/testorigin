export class NestedViewerExtension extends Autodesk.Viewing.Extension {
  public static extensionName: string = 'NestedViewerExtension';
  _filter: any;
  _crossSelection: boolean;
  _group: any;
  _button: any;
  _panel: any;
  _onModelLoaded: () => void;
  ...
  constructor(viewer: Autodesk.Viewing.GuiViewer3D, options:any) {
      ...
      options = options || {};
      this._filter = options.filter || ['2d','3d'];// ['2d', '3d']; default
  ...
  isSelectionSame(sel1: any[], sel2: any[]) {
...
class NestedViewerPanel extends Autodesk.Viewing.UI.DockingPanel {
  ...
  _viewer: any;
  _manifest: any;
  constructor(viewer: Autodesk.Viewing.GuiViewer3D, filter: any, crossSelection: boolean) {
  ...
  override initialize() {
    ...
    //  this.footerInstance.resizeCallback = (width: any, height: any) => {
  ...
  override setVisible(show: boolean) {
  ...
  _updateDropdown() {
      const onDocumentLoadSuccess = (doc:any) => {
          this._manifest = doc;
          const filterGeom = (geom:any) => this._filter.indexOf(geom.data.role) !== -1;
          const geometries = doc.getRoot().search({ type: 'geometry' }).filter(filterGeom);
          if (geometries.length > 0) {
              this._overlay.style.display = 'none';
              this._dropdown.innerHTML = geometries.map(function (geom:any) {
      ...
      this._dropdown.innerHTML = '';
      this.urn='dXJuOmFkc2sub2JqZWN0czpvcy5vYmplY3Q6Y2FyZG9uaXYvUFRHXzIwMjIwNTI2Lm53ZA==' // aqui se coloca la urn que se desea visualizar 
