export class SummaryPanel extends Autodesk.Viewing.UI.PropertyPanel {
  ...
  async update(model:any, dbids:any, propNames:any) {
      ...
      for (const propName of propNames) {
          const initialValue:any = { sum: 0, count: 0, min: -Infinity, max: Infinity };
          const aggregateFunc = (aggregate:any, value: any, property: any) => {
              return {
                  count:Number = aggregate.count + 1,
                  sum: Number = aggregate.sum + value,
                  min: Math.min(aggregate.min, value),
                  max : Math.max(aggregate.max, value),
                  units: Number =property.units,
                  precision:Number = property.precision
              };
          };

          const { sum, count, min, max, units, precision }:any = await this.aggregatePropertyValues(model, dbids, propName, aggregateFunc, initialValue);
          if (count > 0) {
  ...
  async aggregatePropertyValues(model:any, dbids:any, propertyName:any, aggregateFunc:any, initialValue = 0) {
